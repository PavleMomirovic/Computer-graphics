...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	case(VK_UP):
		...
		if (m_glRenderer.alpha > 90) m_glRenderer.alpha = 90;
		break;
	case(VK_DOWN):
		m_glRenderer.alpha -= 5;
		if (m_glRenderer.alpha < -10) m_glRenderer.alpha = -10;
		break;
	case(VK_LEFT):
		m_glRenderer.beta += 5;
		break;
	case(VK_RIGHT):
		...
		break;
	case(VK_ADD):
		m_glRenderer.dist /= 1.1;
		if (m_glRenderer.dist < 10) m_glRenderer.dist = 10;
		break;
	case(VK_SUBTRACT):
		m_glRenderer.dist *= 1.1;
		if (m_glRenderer.dist >50) m_glRenderer.dist = 50;
		break;
	default:
		break;
	}
